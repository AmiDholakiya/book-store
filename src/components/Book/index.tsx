...
const Book = ({ book, handleChange }: BookTypeAsProps) => {
    ...
    const handleLocalChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (e.target.name === "price" && !e.target.value.match(priceRegEx)) setShow(true)
        else setFormData((prevState) => ({ ...prevState, [e.target.name]: e.target.value }));
    }
    ...
    return <div className="book">
        ...
        <ToastCustom show={show} message="Invalid Price Value !" variant="warning" onClose={()=>setShow(false) }  />
    </div>
}
